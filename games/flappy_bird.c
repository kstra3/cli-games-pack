...
void flappy_bird_bird_flap(void) {
    if (game.bird.alive) {
        game.bird.velocity_y = FLAP_STRENGTH;
        game.total_flaps++;
        game.bird.animation_frame = 0; // Reset to flapping up animation
...
void flappy_bird_spawn_pipe(void) {
    for (int i = 0; i < MAX_PIPES; i++) {
        if (!game.pipes[i].active) {
            ...
            game.pipes[i].gap_y = SKY_Y + 2 + (rand() % (GROUND_Y - SKY_Y - game.gap_size - 4));
            game.pipes[i].gap_size = game.gap_size;
...
bool flappy_bird_check_collisions(void) {
    ...
    for (int i = 0; i < MAX_PIPES; i++) {
        if (game.pipes[i].active) {
            ...
            if (bird_x >= game.pipes[i].x && bird_x < game.pipes[i].x + PIPE_WIDTH) {
                // Check if bird is in the gap
                if (bird_y < game.pipes[i].gap_y || bird_y >= game.pipes[i].gap_y + game.pipes[i].gap_size) {
                    // Collision!
                    if (game.sound_enabled) {
...
void flappy_bird_draw_ground(void) {
    for (int y = GROUND_Y; y < SCREEN_HEIGHT - 1; y++) {
        for (int x = 0; x < SCREEN_WIDTH; x += 4) {
            flappy_bird_draw_to_buffer(x, y, "▓▓▓▓");
        }
    }
}
...
void flappy_bird_draw_hud(void) {
    ...
    snprintf(hud_line, sizeof(hud_line), 
             " SCORE: %03d    BEST: %03d    PIPES: %02d ", 
             game.score, game.high_score, game.pipes_passed);
    flappy_bird_draw_to_buffer(0, 0, hud_line);
    
    // Mode and controls
    snprintf(hud_line, sizeof(hud_line), 
             " MODE: CLASSIC    [SPACE] Flap  [P] Pause  [ESC] Exit ");
