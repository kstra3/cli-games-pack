...
#define MAX_GAME_SPEED 25
#define DAY_NIGHT_CYCLE 700
...
static int obstacle_widths[OBSTACLE_COUNT] = {1, 3, 3, 3, 3, 3};
static int obstacle_heights[OBSTACLE_COUNT] = {2, 2, 2, 1, 1, 2};
...
void dino_runner_handle_input(void) {
    if (KBHIT()) {
        ...
        switch (key) {
            case ' ': // Space - Jump
                if (game.dino.on_ground && !game.game_over) {
...
void dino_runner_update_dino(void) {
    ...
    // Handle duck timer
    if (game.dino.duck_timer > 0) {
        game.dino.duck_timer--;
    ...
    if (!game.dino.on_ground) {
        ...
        if (game.dino.y >= DINO_START_Y) {
            ...
            game.dino.velocity_y = 0;
            game.dino.on_ground = true;
            game.dino.state = (game.dino.duck_timer > 0) ? DINO_DUCKING : DINO_RUNNING;
            dino_runner_play_sound("*thud*");
        }
    }
    
    // Running animation
    if (game.dino.state == DINO_RUNNING && game.dino.animation_timer % 8 == 0) {
        game.dino.animation_frame = (game.dino.animation_frame + 1) % 2;
    }
    
    // Ducking animation
    if (game.dino.state == DINO_DUCKING && game.dino.animation_timer % 6 == 0) {
        game.dino.animation_frame = (game.dino.animation_frame + 1) % 2;
    }
}
    ...
    if (spawn_timer <= 0) {
        ...
        for (int i = 0; i < MAX_OBSTACLES; i++) {
            if (!game.obstacles[i].active) {
                ...
                if (game.score < 500) {
                    game.obstacles[i].type = (rand() % 2 == 0) ? OBSTACLE_SMALL_CACTUS : OBSTACLE_LARGE_CACTUS;
                } else if (game.score < 1000) {
                    game.obstacles[i].type = rand() % 4; // Include rocks and birds
                } else {
                    game.obstacles[i].type = rand() % OBSTACLE_COUNT; // All types
                }
                
                // Set position based on type
                if (game.obstacles[i].type == OBSTACLE_BIRD_HIGH) {
                    game.obstacles[i].y = GROUND_Y - 8;
                } else if (game.obstacles[i].type == OBSTACLE_BIRD_LOW) {
        ...
        // Next spawn time (gets shorter as speed increases)
        spawn_timer = 80 - (int)(game.game_speed * 2);
...
void dino_runner_check_collisions(void) {
    for (int i = 0; i < MAX_OBSTACLES; i++) {
        if (game.obstacles[i].active) {
            ...
            // Dino collision box (smaller for fair gameplay)
            float dino_x = game.dino.x + 1;
            float dino_y = game.dino.y + 1;
            float dino_w = (game.dino.state == DINO_DUCKING) ? 3 : 2;
            float dino_h = (game.dino.state == DINO_DUCKING) ? 1 : 2;
            ...
            // Check for close calls (near miss)
...
void dino_runner_draw_dino(void) {
    ...
    switch (game.dino.state) {
        case DINO_RUNNING:
            sprite = dino_running_sprites[game.dino.animation_frame];
            break;
        case DINO_JUMPING:
            sprite = dino_jumping_sprite;
            break;
        case DINO_DUCKING:
            sprite = dino_ducking_sprites[game.dino.animation_frame];
            break;
        case DINO_DEAD:
